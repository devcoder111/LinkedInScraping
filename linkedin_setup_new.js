...
var number_of_instances = 1;
	...
	var functions = ['geturl','privacy','details','avatar','experience','education','skillset']
...
var bots = tress(function(bot,callback){
	...
	function login(){
		...
		needle.get(url, options, function(err, resp){
			...
			if ((err == null) && resp.statusCode === 200) {
				...
				var url = 'https://www.linkedin.com/uas/login-submit?loginSubmitSource=AUTH_WALL'

				options['cookies'] = ck
				options['headers'].cookie = 'bcookie="v=2&8f13d318-b075-43f1-8e97-2c21a08634d5"; bscookie="v=1&20190425170302c52d26b2-99e6-44a1-8d0b-43f96cf91ae7AQFxmNWybZ_9SIiWe8Ocp1KJyulQoOe-"; _ga=GA1.2.1379442034.1556211788; aam_uuid=66850492915209532473998933754781389196; _guid=41ac5586-2681-4676-8cf8-4a5374e3bddf; li_oatml=AQHGVUBo73_XKQAAAWpVrCCNbynNOOfm6ewLq5SfCswaEpS5GMewJ54IQTLKIR3fdMGG6cccypJcWBRLkBxGJ0KaYf5Bz8En; visit=v=1&M; _lipt=CwEAAAFqaOaaQbOiB9kul0TjpeBG0J9IP_pY2AFPYmg-r8VfLNi133IRacdpsHxDoU5bBSRzXLDhKvVj90_-OoMFd3u39dTj4JLDa22qR_Znnt0JpZlyVhVRQSiJ-Ms9_ACGgdJyCcG65wEWolKE35ltXT9UpvxGjsJ6NUnHPOZEdIzj8SCAT6iNQILQwB33mgsg0wsrjCl7Ed-BWLgbgzEf8z-pAy1XN-3cv5KACFqC85Bk1C--owNV8rutyMjP0sMM0bc8X32y3lrlRAu1a_WS2SeT7k3ccg; AMCVS_14215E3D5995C57C0A495C55%40AdobeOrg=1; AMCV_14215E3D5995C57C0A495C55%40AdobeOrg=-1303530583%7CMCIDTS%7C18016%7CMCMID%7C66319021140297462604012775629687792199%7CMCAAMLH-1557162505%7C7%7CMCAAMB-1557162505%7CRKhpRz8krg2tLO6pguXWp5olkAcUniQYPHaMWWgdJ3xzPWQmdj0y%7CMCOPTOUT-1556564905s%7CNONE%7CvVersion%7C3.3.0%7CMCCIDH%7C-1613305357; UserMatchHistory=AQIzWx6SEuq6bgAAAWpqH367HIf_M3uB8orZCYd2DavzdwxZcZNUTN-enVA0SkDJjGIeyoCLJ9qmhDjs11S1h3ENOVvlkPEdCJ1rfhH5U6-S_I6C3nppPiFrWDb8cMo1i52tbeh2kk3MJ56mPepgnkp1rWR2x-s_Q3caeKGBKu4jbKkLj37CkGDVusa2c8PBXr2DCV5a6Q; JSESSIONID="ajax:4265582793435635918"; fid=AQH0QbUNhydzQQAAAWpqOcS8Wu4eJCFL2Rpbr6UbVPv5-SGieaPihll01Y2T8Xd8gIBjB1R6Wkv_Ng; lang=v=2&lang=es-es; lidc="b=VGST05:g=1314:u=1:i=1556560331:t=1556646731:s=AQEDKQQlhTD5YB80csTxkYMS81AgiN3S"; fcookie=AQGUeBkpio3i2wAAAWpqOdEdF6DZ_4xRZAebKwX-25WXui4TYNdOi0tY3mKjADSdQ4xip9CaTpFWkNCa_BcjdYfpNBSqsemf1IKJHOGrJmVah8hN65T-j6RCIMQNp-hsLzDpuBqq3t_Qs7-IKHlA6l7IBudw9bShGgWx25udlMdqw3vaeZbKApfRmhMPBzCGw75XU2RV3kp-jUkHg66V03RPn2pR-ijfF0FySvZwqvJIMtl9iTZhbvlrqgcfOR3vftW6dGgytVhBIFauhsL6nASBWvgTcUNbJX/J1Xy2NP0z2rMZIUPladRhxeWD6J9UxvXuFyg11J4bFGwBCPmaKqiMJFBXlwCqxm7u5A=='
	...
	function privacy(){
		...
		var data_1 = {
			...
			'cancelUrl': 'https://www.linkedin.com/settings/?tab=account&trk=psettings-videos-cancel',
			'redirectUrl': 'https://www.linkedin.com/settings/?tab=account&trk=psettings-videos-redirect',
		...
		var data_6 = {
			...
			'handleType': 'EMAIL',
			'privacyOptions': 'JUST_ME,FIRST_DEGREE_CONNECTIONS,SECOND_DEGREE_CONNECTIONS,EVERYONE,$UNKNOWN',
			'device': 'DESKTOP',
			'initialFetch': true,
			'hasSuccess': false,
			'errors': '[object Object]',
		...
		function vids(){
			needle.post('https://www.linkedin.com/psettings/videos', data_1, options_2, function(err, resp){
				if ((err == null) && resp.statusCode === 200) {
					done++
		...
		function visibility(){
			needle.post('https://www.linkedin.com/psettings/connections-visibility', data_2, options_2, function(err, resp){
				if ((err == null) && resp.statusCode === 200) {
					done++
					eventEmitter.emit('done');
				} else {
		...
		function broadcast(){
			needle.post('https://www.linkedin.com/psettings/activity-broadcast', data_3, options_2, function(err, resp){
				if ((err == null) && resp.statusCode === 200) {
					done++
					eventEmitter.emit('done');
				} else {
		...
		function browse_map(){
			needle.post('https://www.linkedin.com/psettings/browse-map', data_4, options_2, function(err, resp){
				if ((err == null) && resp.statusCode === 200) {
					done++
					eventEmitter.emit('done');
				} else {
		...
		function meet_team(){
			needle.post('https://www.linkedin.com/psettings/meet-the-team', data_5, options_2, function(err, resp){
				if ((err == null) && resp.statusCode === 200) {
					done++
					eventEmitter.emit('done');
				} else {
		...
		function emails(){
			needle.post('https://www.linkedin.com/psettings/privacy/email', data_6, options_2, function(err, resp){
				if ((err == null) && resp.statusCode === 200) {
					done++
					eventEmitter.emit('done');
				} else {
		...
		function post_finish(){
			if(done >= 6){
				...
				writeLog(`Privacy settings done for Bot #${botdb.indexOf(bot) + 1}`,'Info:')
				streamline()
			}
		}
	...
	function details(){
		...
		options_2['headers'] = {
			...
			'Host': 'www.linkedin.com',
			'Origin': 'https://www.linkedin.com',
			'Referer': `${tag.replace('https://www.linkedin.com/in/','')}/edit/topcard/`,
			'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Safari/537.36',
			'X-LI-Lang': 'en_US',
			'X-LI-Track': '{"clientVersion":"1.2.2370","osName":"web","timezoneOffset":8,"deviceFormFactor":"DESKTOP","mpName":"voyager-web"}',
			'X-Requested-With': 'XMLHttpRequest',
		...
		needle.get(`${tag}/edit/topcard/`,options_2,function(err, resp){
			if ((err == null) && resp.statusCode === 200) {
				...
				try{
				var id = resp.body.match(/urn:li:fs_miniProfile:([a-zA-Z0-9_-]*)&quot;,/)[1]
				var ver = resp.body.match(/versionTag&quot;:&quot;([0-9]+)/)[1]
				var inst = resp.body.match(/(urn:li:page:d_flagship3_profile_self_edit_top_card[a-zA-Z0-9_;\/]*)/)[1]
				} catch(err){
					fs.writeFileSync('dettest.html',resp.body,function(){})
					log_err(`In function Details for Bot #${botdb.indexOf(bot) + 1}`)
					writeLog(`In function Details for Bot #${botdb.indexOf(bot) + 1}`,'Error:')
					details()
					return
				}
				...
				options_2['headers'] = {
					...
					'X-LI-Track': '{"clientVersion":"1.2.2430","osName":"web","timezoneOffset":8,"deviceFormFactor":"DESKTOP","mpName":"voyager-web"}',
					'X-Requested-With': 'XMLHttpRequest',
					'X-RestLi-Protocol-Version': '2.0.0'
				}
				...
				needle.post(url,JSON.stringify(data),options_2,function(err, resp){
					if ((err == null) && (resp.statusCode === 200 || resp.statusCode === 202)) {
						log(`Now the name for Bot #${botdb.indexOf(bot) + 1} is - ${bot.firstname} ${bot.lastname}`)
						writeLog(`Now the name for Bot #${botdb.indexOf(bot) + 1} is - ${bot.firstname} ${bot.lastname}`,'Info:')
						log(`Now the location for Bot #${botdb.indexOf(bot) + 1} is - US`)
						writeLog(`Now the location for Bot #${botdb.indexOf(bot) + 1} is - US`,'Info:')
						tag = ''
						streamline()
					} else {
						//log_err(err)
		...
		needle.get(`${tag}/edit/topcard/`,options_2,function(err, resp){
			if ((err == null) && resp.statusCode === 200) {
				...
				try{
					...
					log_err(`In function Avatar for Bot #${botdb.indexOf(bot) + 1}, repeating`)
					writeLog(`In function Avatar for Bot #${botdb.indexOf(bot) + 1}, repeating`,'Error:')
					avatar()
					return
				}
				...
				var data_1 = {"mediaUploadType":"PROFILE_ORIGINAL_PHOTO","fileSize":img_size,"filename":`${bot.img}.jpg`}
				var data_2 = {"mediaUploadType":"PROFILE_DISPLAY_PHOTO","fileSize":img_size}

				needle.post(`https://www.linkedin.com/voyager/api/voyagerMediaUploadMetadata?action=upload`,JSON.stringify(data_1),options_2,function(err, resp){
					if ((err == null) && resp.statusCode === 200) {
						var url_1 = resp.body.value.singleUploadUrl
						var urn_1 = resp.body.value.urn
						needle.post(`https://www.linkedin.com/voyager/api/voyagerMediaUploadMetadata?action=upload`,JSON.stringify(data_2),options_2,function(err, resp){
							if ((err == null) && resp.statusCode === 200) {
								...
								needle.put(url_1,fs.createReadStream(`${bot.img}.jpg`),options_2,function(err, resp){
									if ((err == null) && resp.statusCode === 201) {

										options_2.headers['path'] = url_2.replace('https://www.linkedin.com','')
			
										needle.put(url_2,fs.createReadStream(`${bot.img}.jpg`),options_2,function(err, resp){
											if ((err == null) && resp.statusCode === 201) {
												...
												needle.post(url,JSON.stringify(data_4),options_2,function(err, resp){
													if ((err == null) && (resp.statusCode === 200 || resp.statusCode === 202)) {
														log(`Avatar was set for Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`)
														writeLog(`Avatar was set for Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`,'Info:')
														tag = ''
														streamline()
													} else {
														//log_err(err)
		...
		function deletePosition(set_hl = false, bAdd = false){
			...
			needle.get(`${tag}`,sp_ots,function(err, resp){
				if ((err == null) && resp.statusCode === 200) {
					...
					var ver = resp.body.match(/versionTag&quot;:&quot;([0-9]+)/)[1]
					var id = resp.body.match(/urn:li:fs_miniProfile:([a-zA-Z0-9_-]*)&quot;,/)[1]
					var job = resp.body.match(/quot;urn:li:fs_position:\([a-zA-Z0-9_-]*,([0-9]+)\)&quot;/)

					if(job !== null){
						job = job[1]
					} else {
						log(`All previous jobs experience deleted for Bot #${botdb.indexOf(bot) + 1}`)
						writeLog(`All previous jobs experience deleted for Bot #${botdb.indexOf(bot) + 1}`,'Info:')
						if (bAdd) {
							addPosition(set_hl)
						}
						return;
					...
					needle.get(`${tag}/edit/position/${job}/`,sp_ots,function(err, resp){
						if ((err == null) && resp.statusCode === 200) {

							try{
								var inst = resp.body.match(/(urn:li:page:d_flagship3_profile_self_edit_position[a-zA-Z0-9_;+\/]*)/)[1] + '=='
							} catch(err){
								log_err(`In function Experience for Bot #${botdb.indexOf(bot) + 1}, repeating`)
								writeLog(`In function Experience for Bot #${botdb.indexOf(bot) + 1}, repeating`,'Error:')
								experience()
								return
							}
							...
							sp_ots['headers'] = {
								...
								'x-li-lang': 'en_US',
								'x-li-page-instance': inst,
								'x-li-track': '{"clientVersion":"1.2.4258","osName":"web","timezoneOffset":8,"deviceFormFactor":"DESKTOP","mpName":"voyager-web"}',
								'x-requested-with': 'XMLHttpRequest',
								'x-restli-protocol-version': '2.0.0'
							}

							needle.delete(del_url,{},sp_ots,function(err, resp){
								if ((err == null) && (resp.statusCode === 200 || resp.statusCode == 204)) {
		...
		function set_headline() {
			...
			needle.get(`${tag}/edit/topcard/`,options_2,function(err, resp){
				if ((err == null) && resp.statusCode === 200) {
					...
					var body = resp.body;
					var stdate = body.match(/{&quot;startDate&quot;:{&quot;year&quot;:([0-9]+)/);
					if (stdate !== null) {
						stdate = stdate[1];
					}
					else {
						log(`No jobs to set headline #${botdb.indexOf(bot) + 1}`)
						writeLog(`No jobs to set headline #${botdb.indexOf(bot) + 1}`,'Info:')
						tag = ''
						streamline()
						return
					}
					...
					var a  = new RegExp('quot;title&quot;:&quot;([a-zA-Z_ -.]*)&quot;,&quot;companyUrn&quot;:&quot;urn:li:fs_miniCompany:' + cpid);
					var jobtitle = body.match(a)
					var b = new RegExp('urn:li:fs_miniCompany:'+ cpid + '&quot;,&quot;name&quot;:&quot;([a-zA-Z_ -.&;]*)&quot;,')
		...
		function addPosition(set_hl = false){
			...
			needle.get(`${tag}/edit/position/new/`,options_2,function(err, resp){
				if ((err == null) && resp.statusCode === 200) {
					...
					var id = resp.body.match(/urn:li:fs_miniProfile:([a-zA-Z0-9_-]*)&quot;,/)[1]
					var ver = resp.body.match(/versionTag&quot;:&quot;([0-9]+)/)[1]
					var inst = resp.body.match(/(urn:li:page:d_flagship3_profile_self_add_position[a-zA-Z0-9_;\/]*)/)[1]
					...
					needle.get(`https://www.linkedin.com/voyager/api/typeahead/hits?q=federated&query=${encodeURIComponent(job)}&shouldUseSchoolParams=false&types=List(COMPANY)`,options_2,function(err, resp){
						if ((err == null) && resp.statusCode === 200) {
							...
							for(entity of comp.included){
								if(entity.name && entity.name.toUpperCase() == job.toUpperCase()){
									comp = entity.objectUrn.replace('company','fs_miniCompany')
									passed = true
									break
								}
							}
							...
							if(!passed){
								for(entity of comp.included){
									if(entity.name && entity.name.toUpperCase().indexOf(job.toUpperCase()) !== -1){
										comp = entity.objectUrn.replace('company','fs_miniCompany')
										job = entity.name
										passed = true
										break
							...
							if(!passed){
								log_err(`Error while getting urn parameter for job for Bot #${botdb.indexOf(bot) + 1} - ${bot.email}`)
							...
							if(current){
								data.timePeriod['startDate'] = {"year":parseInt(dates[curDate])}
							} else {
								data.timePeriod['startDate'] = {"year":parseInt(dates[curDate+1])}
								data.timePeriod['endDate'] = {"year":parseInt(dates[curDate])}
							}
								...
								if ((err == null) && resp.statusCode === 201) {
									...
									if(positions < 4){
										...
										if (set_hl) {
											...
											log(`Working experience was filled by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`)
														writeLog(`Working experience was filled by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`,'Info:')
														tag = ''
														streamline()
										}
									}
	...
	function education(){
		...
		needle.get(`${tag}/edit/education/new/`,options_2,function(err, resp){
			if ((err == null) && resp.statusCode === 200) {
				...
				var id = resp.body.match(/urn:li:fs_miniProfile:([a-zA-Z0-9_-]*)&quot;,/)[1]
				var ver = resp.body.match(/versionTag&quot;:&quot;([0-9]+)/)[1]
				var inst = resp.body.match(/(urn:li:page:d_flagship3_profile_self_add_education[a-zA-Z0-9_;\/]*)/)[1]
				...
				var url = `https://www.linkedin.com/voyager/api/typeahead/hits?q=federated&query=${encodeURIComponent(school)}&shouldUseSchoolParams=true&types=List(SCHOOL)`
				...
				needle.get(url,options_2,function(err, resp){
					if ((err == null) && resp.statusCode === 200) {
						...
						for(entity of comp.included){
							if(entity.schoolName && entity.schoolName.toUpperCase() == school.toUpperCase()){
						...
						if(!passed){
							for(entity of comp.included){
								if(entity.schoolName && entity.schoolName.toUpperCase().indexOf(school.toUpperCase()) !== -1){
									comp = entity.entityUrn
									school = entity.name
									passed = true
									break
						...
						if(!passed){
							log_err(`Error while getting urn parameter for school for Bot #${botdb.indexOf(bot) + 1} - ${bot.email}`)
						...
						needle.post(url,JSON.stringify(data),options_2,function(err, resp){
							if ((err == null) && resp.statusCode === 201) {
								log(`Education was filled by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`)
								writeLog(`Education was filled by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`,'Info:')
								tag = ''
								streamline()
							} else {
								//log_err(err)
	...
	function skillset(deleted){
		...
		needle.get(`${tag}/detail/skills/add/`,options_2,function(err, resp){
			if ((err == null) && resp.statusCode === 200) {
				...
				var id = resp.body.match(/urn:li:fs_miniProfile:([a-zA-Z0-9_-]*)&quot;,/)[1]
				var ver = resp.body.match(/versionTag&quot;:&quot;([0-9]+)/)[1]
				var inst = resp.body.match(/(urn:li:page:d_flagship3_profile_self_skill_typeahead[a-zA-Z0-9_;\/]*)/)[1]
				...
				if(!deleted){
					...
					options_2.headers = {
						...
						'x-li-lang': 'en_US',
						'x-li-page-instance': inst,
						'x-li-track': '{"clientVersion":"1.2.2535","osName":"web","timezoneOffset":0,"deviceFormFactor":"DESKTOP","mpName":"voyager-web"}',
						'x-requested-with': 'XMLHttpRequest',
						'x-restli-protocol-version': '2.0.0'
					}
					...
					needle.post(url,JSON.stringify(data),options_2,function(err, resp){
						if ((err == null) && resp.statusCode === 200) {
							...
							log(`Old skills were deleted by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`)
							writeLog(`Old skills were deleted by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`,'Info:')
					...
					options_2.headers = {
						...
						'x-li-lang': 'en_US',
						'x-li-page-instance': inst,
						'x-li-track': '{"clientVersion":"1.3.323","osName":"web","timezoneOffset":0,"deviceFormFactor":"DESKTOP","mpName":"voyager-web"}',
						'x-requested-with': 'XMLHttpRequest',
					...
					needle.post(url,JSON.stringify(data),options_2,function(err, resp){
						if ((err == null) && resp.statusCode === 200 ) {
							log(`20 skills were added by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`)
							writeLog(`20 skills were added by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`,'Info:')
							tag = ''
							streamline()
						} else {
							log_err(err)
	...
	function changepw(){
		...
		needle.post('https://www.linkedin.com/psettings/change-password', data, options_2, function(err, resp){
			if ((err == null) && resp.statusCode === 200) {
				log(`Password was changed by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`)
				writeLog(`Password was changed by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`,'Info:')
				streamline()
			} else {
				//log_err(err)
				changepw()
	...
	function invitesettings(){
		...
		needle.post('https://www.linkedin.com/psettings/invite-receive', data, options_2, function(err, resp){
			if ((err == null) && resp.statusCode === 200) {
				log(`Invite settings changed by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`)
				writeLog(`Invite settings changed by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`,'Info:')
				streamline()
			} else {
				//log_err(err)
				invitesettings()
	...
	function acceptinvites(){
		...
		needle.get('https://www.linkedin.com/mynetwork/invitation-manager/', options_2, function(err, resp){
			if ((err == null) && resp.statusCode === 200) {
				
				var inst = resp.body.match(/(urn:li:page:d_flagship3_people_invitations[a-zA-Z0-9_;\/]*)/)[1]

				var invites = resp.body.replace(/&quot;/g,'').match(/invitation:(urn:li:fs_relInvitation:[0-9]+)/g)
				var shared = resp.body.replace(/&quot;/g,'').match(/sharedSecret:([a-zA-Z0-9]+)/g)

				if(invites == null || shared == null){
					log(`All invites accepted by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`)
					writeLog(`All invites accepted by Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`,'Info:')
					streamline()
				} else {
					if(invites.length !== shared.length){
						log_err(`Something wrong with invitations for Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`)
						writeLog(`Something wrong with invitations for Bot #${botdb.indexOf(bot) + 1} - Profile: ${bot.firstname} ${bot.lastname}`,'Error:')
						acceptinvites()
					} else {
						...
						options_2['headers'] = {
							'authority': 'www.linkedin.com',
							'method': 'POST',
							'path': '/voyager/api/relationships/invitations?action=closeInvitations',
							'scheme': 'https',
							'accept': 'application/vnd.linkedin.normalized+json+2.1',
							'accept-encoding': 'gzip, deflate, br',
							'accept-language': 'ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7',
							...
							'csrf-token': bot.csrf,
							'origin': 'https://www.linkedin.com',
							'referer': 'https://www.linkedin.com/mynetwork/invitation-manager/',
							'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Safari/537.36',
							'x-li-lang': 'en_US',
							'x-li-page-instance': inst,
							'x-li-track': '{"clientVersion":"1.2.2535","osName":"web","timezoneOffset":0,"deviceFormFactor":"DESKTOP","mpName":"voyager-web"}',
						...
						needle.post('https://www.linkedin.com/voyager/api/relationships/invitations?action=closeInvitations', JSON.stringify(data), options_2, function(err, resp){
							if ((err == null) && resp.statusCode === 200) {
								acceptinvites()
							} else {
								//log_err(err)
